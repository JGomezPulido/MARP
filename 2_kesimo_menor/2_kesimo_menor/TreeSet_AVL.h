...
protected:
    ...
    struct TreeNode {
        ...
        TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
            int alt = 1, int ord = 1) : elem(e), iz(i), dr(d), altura(alt), tam_i(ord) {}
    };
...
protected:
    ...
    T kesimoMenor(int orden, Link r) const{
        if (r == nullptr) {
        ...
        else if (orden > r->tam_i) {
            return kesimoMenor(orden - r->tam_i, r->dr);
        }
        else {
            return kesimoMenor(orden, r->iz);
        }
    }
